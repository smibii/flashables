...
vec3 blend(vec3 lightColor, vec3 baseColor) {
...
void main()
{
    ...
    if (hasSurface)
    {
        ...
        if (distanceToLight < LightRadius && spot > 0.0)
        {
            ...
            contribution = blend(lightColor, baseColor);
        }
    }
    ...
    if (LightVolumetric >= 0.5)
    {
         ...
         */
        vec3 volumetricColor = vec3(1.0) - (vec3(1.0) - contribution) * (vec3(1.0) - volumetric);
